    ...
    async with async_session() as session:
        ...
        if result.scalar() is None:
            ...
            try:
                ...
                logger.warning("Файл categories.json не найден! Используем пустую структуру.")
                initial_structure = {}
            except json.JSONDecodeError:
                logger.warning("Ошибка чтения JSON! Используем пустую структуру.")
                initial_structure = {}
                ...
                for name, data in structure.items():
                    ...
                    for item_data in items_list:
                        ...
                        if init_file and os.path.exists(init_file):
                            ...
                            sanitized_stack = ["".join(c for c in n if c.isalnum() or c in (' ', '_', '-')).strip() for
                                               n in current_path_stack]
                            ...
                            if not safe_name: safe_name = "item"
                        ...
                        if item:
                            ...
                            if need_copy:
                                if os.path.exists(init_file):
                                    # Copy logic
                                    sanitized_stack = [
                                        "".join(c for c in n if c.isalnum() or c in (' ', '_', '-')).strip() for n in
                                        current_path_stack]
                                    target_dir = os.path.join("files", *sanitized_stack)
                                    os.makedirs(target_dir, exist_ok=True)
                                    ...
                                    ext = os.path.splitext(init_file)[1]
                                    safe_name = "".join(
                                        c for c in item_name if c.isalnum() or c in (' ', '_', '-')).strip()
                                    if not safe_name: safe_name = "item"
                                    target_filename = f"{safe_name}{ext}"
                                    target_path = os.path.join(target_dir, target_filename)
                                    ...
                                    logger.info(f"WARNING: init_file not found: {init_file} (CWD: {os.getcwd()})")
                                    # Debug parent dir
                                    pdir = os.path.dirname(init_file)
                                    if os.path.exists(pdir):
                                        logger.info(f"Dir {pdir} exists. Content: {os.listdir(pdir)}")
                                    else:
                                        logger.info(f"Dir {pdir} NOT found.")
